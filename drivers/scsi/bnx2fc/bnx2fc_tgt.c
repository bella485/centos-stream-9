					...
					struct bnx2fc_rport *tgt)
{
	...
	tgt->sq_dma = dma_map_single(&hba->pcidev->dev, tgt->sq, tgt->sq_mem_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->sq_dma))) {
		printk(KERN_ERR PFX "unable to map SQ memory %d\n",
			tgt->sq_mem_size);
		goto sq_map_failure;
	}
	...
	tgt->cq_dma = dma_map_single(&hba->pcidev->dev, tgt->cq, tgt->cq_mem_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->cq_dma))) {
		printk(KERN_ERR PFX "unable to map CQ memory %d\n",
			tgt->cq_mem_size);
		goto cq_map_failure;
	}
	...
	tgt->rq_dma = dma_map_single(&hba->pcidev->dev, tgt->rq, tgt->rq_mem_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->rq_dma))) {
		printk(KERN_ERR PFX "unable to map RQ memory %d\n",
			tgt->rq_mem_size);
		goto rq_map_failure;
	}
	...
	tgt->rq_pbl_dma = dma_map_single(&hba->pcidev->dev, tgt->rq_pbl, tgt->rq_pbl_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->rq_pbl_dma))) {
		printk(KERN_ERR PFX "unable to map RQ PBL memory %d\n",
			tgt->rq_pbl_size);
	...
	tgt->xferq_dma = dma_map_single(&hba->pcidev->dev, tgt->xferq, tgt->xferq_mem_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->xferq_dma))) {
		printk(KERN_ERR PFX "unable to map XFERQ memory %d\n",
			tgt->xferq_mem_size);
	...
	tgt->confq_dma = dma_map_single(&hba->pcidev->dev, tgt->confq, tgt->confq_mem_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->confq_dma))) {
		printk(KERN_ERR PFX "unable to map CONFQ memory %d\n",
			tgt->confq_mem_size);
	...
	tgt->confq_pbl_dma = dma_map_single(&hba->pcidev->dev, tgt->confq_pbl, tgt->confq_pbl_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->confq_pbl_dma))) {
		printk(KERN_ERR PFX "unable to map CONFQ PBL memory %d\n",
			tgt->confq_pbl_size);
	...
	tgt->conn_db_dma = dma_map_single(&hba->pcidev->dev, tgt->conn_db, tgt->conn_db_mem_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->conn_db_dma))) {
		printk(KERN_ERR PFX "unable to map conn db memory %d\n",
			tgt->conn_db_mem_size);
	...
	tgt->lcq_dma = dma_map_single(&hba->pcidev->dev, tgt->lcq, tgt->lcq_mem_size, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&hba->pcidev->dev, tgt->lcq_dma))) {
		printk(KERN_ERR PFX "unable to map lcq memory %d\n",
			tgt->lcq_mem_size);
...
lcq_map_failure:
	kfree(tgt->lcq);
lcq_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->conn_db_dma, tgt->conn_db_mem_size, DMA_BIDIRECTIONAL);	
conn_db_map_failure:
	kfree(tgt->conn_db);
conn_db_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->confq_pbl_dma, tgt->confq_pbl_size, DMA_BIDIRECTIONAL);	
confq_pbl_map_failure:
	kfree(tgt->confq_pbl);
confq_pbl_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->confq_dma, tgt->confq_mem_size, DMA_BIDIRECTIONAL);	
confq_map_failure:
	kfree(tgt->confq);
confq_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->xferq_dma, tgt->xferq_mem_size, DMA_BIDIRECTIONAL);	
xferq_map_failure:
	kfree(tgt->xferq);
xferq_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->rq_pbl_dma, tgt->rq_pbl_size, DMA_BIDIRECTIONAL);	
rq_pbl_map_failure:
	kfree(tgt->rq_pbl);
rq_pbl_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->rq_dma, tgt->rq_mem_size, DMA_BIDIRECTIONAL);
rq_map_failure:
	kfree(tgt->rq);
rq_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->cq_dma, tgt->cq_mem_size, DMA_BIDIRECTIONAL);
cq_map_failure:
	kfree(tgt->cq);
cq_alloc_failure:
	dma_unmap_single(&hba->pcidev->dev, tgt->sq_dma, tgt->sq_mem_size, DMA_BIDIRECTIONAL);
sq_map_failure:
	kfree(tgt->sq);
sq_alloc_failure:
	return -ENOMEM;
						...
						struct bnx2fc_rport *tgt)
{
	...
	/* Free LCQ */
	if (tgt->lcq) {
		dma_unmap_single(&hba->pcidev->dev, tgt->lcq_dma, tgt->lcq_mem_size, DMA_BIDIRECTIONAL);
		kfree(tgt->lcq);
		tgt->lcq = NULL;
	}
	/* Free connDB */
